...
# search for year
  ...
  #add to dict: tuple_item[1] (boy's name) key, tuple_item[0] (rank) value
  name_rank = {}
  for tuple_item in rank_bname_gname:
    (rank, boyname, girlname) = tuple_item
    name_rank[tuple_item[1]] = tuple_item[0]       
  ...
  #add to dict: tuple_item[2] (girl's name) key, tuple_item[0] (rank) value
  #if girl's name already exist, then retain highest rank
  for tuple_item in rank_bname_gname: 
    if not tuple_item[2] in name_rank.keys():     
      name_rank[tuple_item[2]] = tuple_item[0]    
    elif int(name_rank[tuple_item[2]]) > int(tuple_item[0]): 
      name_rank[tuple_item[2]] = tuple_item[0]
